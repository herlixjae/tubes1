...
//pagination variables
	...
	$count_query   = mysql_query("SELECT COUNT(post_id) AS numrows FROM post_article Where post_article.id_cat='".$_GET['category']."'",$con);
...
FROM (post_article LEFT JOIN komentar ON post_article.post_id = komentar.post_id) LEFT JOIN post_category ON post_article.id_cat = post_category.id_cat Where post_article.id_cat='".$_GET['category']."' GROUP BY post_article.post_id ORDER BY `create_date` DESC LIMIT $offset,$per_page") or die(mysql_error());
...
<div align="center"> 
<?php
echo paginate("?category=".$_GET['category']."&", $page, $total_pages, $adjacents);
?>
</div>
    <div class="clear"></div>
