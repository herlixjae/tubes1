...
FROM (post_article LEFT JOIN komentar ON post_article.post_id = komentar.post_id) LEFT JOIN post_category ON post_article.id_cat = post_category.id_cat Where post_article.id_cat='".$feature_cat_3."' and post_article.status='1' GROUP BY post_article.post_id ORDER BY `create_date` DESC") or die(mysql_error());
